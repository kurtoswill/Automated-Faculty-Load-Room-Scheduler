...
// In Next.js App Router: const { id } = useParams();
// In Pages Router: const { id } = useRouter().query;
...
const ROLE_STYLE: Record<Role, { label: string; badge: string }> = {
    Admin: { label: "Administrator", badge: "bg-amber-50 text-amber-700 border-amber-200" },
    Instructor: { label: "Instructor", badge: "bg-sky-50 text-sky-700 border-sky-200" },
    Student: { label: "Student", badge: "bg-emerald-50 text-emerald-700 border-emerald-200" },
};
    ...
    return (
        <div>
            <input
                ...
                className={`w-full rounded-lg border px-4 py-2.5 text-sm font-medium outline-none transition-all
          ${disabled
                        ? "bg-slate-50 border-slate-200 text-slate-400 cursor-not-allowed"
                        : error
                            ? "border-red-300 bg-red-50 text-slate-800 focus:border-red-400 focus:ring-2 focus:ring-red-100"
...
function ReadOnlyField({ label, value }: { label: string; value: string }) {
    return (
        <div>
            ...
            <span className="block font-mono text-sm text-slate-500 bg-slate-50 border border-slate-200 rounded-lg px-4 py-2.5">
                {value || "—"}
            </span>
        </div>
    );
...
function ConfirmModal({
    ...
    const config = {
        ...
        resetPassword: {
            ...
            confirmClass: "bg-slate-800 hover:bg-amber-500 hover:text-slate-900 text-white",
            icon: (
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
            ...
            {/* Panel */}
            <div className="relative bg-white rounded-2xl shadow-xl border border-slate-200 max-w-sm w-full p-6 flex flex-col gap-5">
                ...
                <div className="flex gap-3">
                    <button onClick={onCancel} disabled={isLoading}
                        className="flex-1 px-4 py-2.5 rounded-lg border border-slate-200 text-sm font-semibold text-slate-700 hover:bg-slate-50 transition-all disabled:opacity-50">
                        Cancel
                    </button>
                    <button onClick={onConfirm} disabled={isLoading}
                        className={`flex-1 px-4 py-2.5 rounded-lg text-sm font-semibold transition-all disabled:opacity-50 inline-flex items-center justify-center gap-2 ${config.confirmClass}`}>
                        {isLoading ? (
                            <>
...
export default function AdminUserDetailPage() {
    // TODO: Get the user ID from router (useParams / useRouter)
    // const { id } = useParams(); // App Router
    // const { id } = useRouter().query; // Pages Router

    // TODO: fetch(`/api/admin/users/${id}`) → setUser(data)
    ...
    async function handleSave() {
        ...
        try {
            ...
            // Optimistically update local state to reflect saved values
    ...
    async function handleModalConfirm() {
        ...
        try {
            if (modal === "deactivate") {
                // TODO: await fetch(`/api/admin/users/${user.id}/deactivate`, { method: "POST" })
                await new Promise((r) => setTimeout(r, 700));
                setUser((u) => ({ ...u, is_active: false }));
            } else if (modal === "reactivate") {
                // TODO: await fetch(`/api/admin/users/${user.id}/reactivate`, { method: "POST" })
                await new Promise((r) => setTimeout(r, 700));
                setUser((u) => ({ ...u, is_active: true }));
            } else if (modal === "resetPassword") {
                // TODO: const res = await fetch(`/api/admin/users/${user.id}/reset-password`, { method: "POST" })
                //       const { temp_password } = await res.json();
                //       setTempPassword(temp_password);
                await new Promise((r) => setTimeout(r, 700));
                setTempPassword("Tmp@" + Math.random().toString(36).slice(2, 10)); // placeholder
            }
            setModal(null);
        } finally {
