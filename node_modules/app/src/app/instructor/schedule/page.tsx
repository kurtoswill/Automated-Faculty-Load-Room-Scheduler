...
function SummaryBar({ grouped }: { grouped: ReturnType<typeof groupBySubject> }) {
  ...
  const classDays     = [...new Set(grouped.flatMap(r => r.days))].length;
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
      {[
        ...
        { label: 'Total Time Blocks', value: grouped.length, color: 'var(--color-text-secondary)', bg: 'var(--color-surface-2)', icon: <IcoClock /> },
      ].map(s => (
...
export default function InstructorSchedulePage() {
  ...
  const [rows, setRows] = useState<ConfirmedClass[]>([]);
  ...
  useEffect(() => {
    (async () => {
      try {
        const [meData, res] = await Promise.all([
          me(),
          api.get<{ success: boolean; data: Array<any> }>('/instructor/schedule'),
        ]);
        setUser(meData);
        if (Array.isArray(res.data) && res.data.length > 0) {
          const mapped: ConfirmedClass[] = res.data.map((r) => ({
            scheduleId: Number(r.id),
            requestId: Number(r.request_id ?? 0),
            dayOfWeek: (r.day_of_week ?? 'Monday') as DayOfWeek,
            timeStart: r.time_start ?? '07:30',
            timeEnd: r.time_end ?? '09:00',
            isActive: Boolean(r.is_active ?? true),
            confirmedAt: r.confirmed_at ?? new Date().toISOString(),
            sectionId: Number(r.section?.id ?? 0),
            sectionName: r.section?.section_name ?? 'N/A',
            semester: r.section?.semester ?? 'Current Semester',
            yearLevel: Number(r.section?.year_level ?? 1),
            expectedStudents: Number(r.section?.expected_students ?? 0),
            courseCode: r.course?.course_code ?? 'N/A',
            courseTitle: r.course?.course_title ?? 'Untitled Course',
            units: Number(r.course?.units ?? 0),
            deptCode: r.course?.department?.code ?? 'N/A',
            roomNumber: r.room?.room_number ?? 'TBA',
            building: r.room?.building ?? 'N/A',
            roomCapacity: Number(r.room?.capacity ?? 0),
  ...
  const grouped = useMemo(() => groupBySubject(rows), [rows]);
  const classDays = useMemo(
    () => DAY_ORDER.filter(d => rows.some(r => r.dayOfWeek === d)),
    [rows],
  );
  ...
  const displayName = user ? `${user.first_name} ${user.last_name}` : 'Instructor';
  const initials = displayName.split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2);

  return (
    <AppShell role="instructor" userName={displayName} pageTitle="My Schedule">
              ...
              {classDays.map(d => (
                <button key={d}
                  ...
                  <span className="ml-1.5 text-[10.5px] opacity-70">
                    ({rows.filter(r => r.dayOfWeek === d).length})
                  </span>
                </button>
              ))}
            </div>
            <DayList rows={rows} day={activeDay} />
          </div>
        )}
