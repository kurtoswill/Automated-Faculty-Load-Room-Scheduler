...
function getValidationMessage(errors?: Record<string, string[]>) {
  const firstField = errors ? Object.keys(errors)[0] : undefined;
  return firstField ? errors?.[firstField]?.[0] : undefined;
}

function persistSession(token: string, role: BackendRole, remember: boolean) {
  if (remember) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!response.success || !response.data) {
        throw new Error(
          getValidationMessage(response.errors) ||
            response.message ||
            "Invalid credentials."
        );
      }
      ...
      if (userRole !== SELECTED_ROLE[role]) {
        throw new Error(
          `This account is registered as ${userRole}. Select ${userRole} to continue.`
        );
      }

      persistSession(response.data.token, userRole as BackendRole, remember);
      router.replace(ROLE_ROUTE[userRole as BackendRole]);
    } catch (err: unknown) {
      setFormState("error");
      // Use the Laravel API error message if available, fallback to default
